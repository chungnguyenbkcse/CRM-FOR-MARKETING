<?php
function getStatesRo($sale_stage, $created_by)
{
    ...
    if ($created_by == "1") {
        ...
        while (($row = $db->fetchByAssoc($result)) != null) {
            foreach ($lead_status_new_dom as $k => $v) {
                if ($k == $row['lead_status_id'] && $row['id'] != 43 && $row['id'] != 44 && $row['id'] != 45) {
                    $list[$k] = $v;
                }
            }
        }
...
function getStatesBuAndMkt($sale_stage, $created_by)
{
    $ck_login_language_20 = $_COOKIE['ck_login_language_20'];
    $lead_status_new_dom = array(
        '' => '',
...
function getStatesSuperMkt($sale_stage, $created_by)
{
    $ck_login_language_20 = $_COOKIE['ck_login_language_20'];
    $lead_status_new_dom = array(
        '' => '',
...
if (isset($_COOKIE['role'])){
    ...
    if ($role == "MKT"){
        $html = "";
            ...
            $states = getStatesBuAndMkt($_GET['id'], $created_by);
            echo print_r($states);
        ...
        # code...
        $created_by = $_GET['created_by'];
        $states = getStatesRo($_GET['id'], $created_by);
        foreach ($states as $k => $v) {
        ...
        $html = "";
            # code...
            $created_by = $_GET['created_by'];
            $states = getStatesSuperMkt($_GET['id'], $created_by);
            echo print_r($states);
            foreach ($states as $k => $v) {
                if ($_GET['lead_status_id'] == "0") {
                    echo $html .= sprintf("<option value='%s' selected>%s</option>", $k, $v);
